...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Get all requests visible to user profile (Todas las solicitudes)
  app.get("/api/requests/all-requests/:userProfile", async (req, res) => {
    try {
      ...
      // For "all-requests", return all requests (no filtering by user permission for approval)
      // This is different from pending-approval which filters by who can approve
      const requests = await storage.getRequests(filters);
      res.json(requests);
    } catch (error) {
